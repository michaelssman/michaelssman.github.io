...
2. 通过Prototype模式创建的对象，不仅仅与原型对象具有相同的结构，还与原型对象具有相同的值。
3. 根据对象克隆深度层次的不同，有浅度克隆与深度克隆。

总结一句话：让一个复杂的对象，具有自我复制功能和统一一套接口。

类具有复制功能，因为类的创建比较复杂，很多属性。
...
2. 一种Person类如 CPlusPlusProgrammer（C++工程师）继承Person类
   ...
   实现构造函数、析构函数、还有clone函数。

   实现clone函数，里面创建一个对象，赋值返回。

   如果成员变量有指针，则需要写set指针变量方法，里面根据大小分配内存创建对象，然后拷贝。

   可以生成CPlusPlusProgrammer对象 然后就可以clone刚才创建的对象去创建对象

3. 客户端 调用CPlusPlusProgrammer的clone
