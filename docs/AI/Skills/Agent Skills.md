...
Skills 对智能体是**渐进式披露**的，这意味着 Skill 的名称和描述始终存在于智能体的上下文窗口中，但直到用户请求与 Skill 的描述匹配时，智能体才会将剩余的指令加载到其上下文中。那时，智能体可能还会根据需要额外加载引用文件和资源文件。
...
每个 Skill 都有一个必需的 SKILL.md 文件，其中包含 YAML 前置元数据，名称、描述和主要指令。主要指令还可以引用其他文件，如脚本、额外的 Markdown 文件以及模板和图片等资源，这些只有在需要时才会加载。
...
如何打包一个 Skill——一个独立的资源，实际上就是一个文件夹，包含指令，同时有意识地管理哪些信息进入上下文窗口，哪些不进入。
...
为了打包这些底层的**提示词**、对话、代码和资源，我们可以利用 Skills。被委派任务的子智能体可以使用 Skills。这些子智能体然后可以通过 MCP 定义的从主智能体那里获取必要的工具。
...
如果发现需要特定的输出格式，不要把所有内容都放在 SKILL.md 中，而是在外部资源或文件中引用。记住，这些模板只有在需要时才会加载。因此，我们可以通过只加载所需数据格式的特定文件来高效使用 token 和上下文窗口。
