...
- 如果current不等于nullNode，则有重复的，抛出异常。

- current等于nullNode，则找到了位置，current是空节点，那么创建current节点，把它挂在父节点上。

- 挂在父节点，然后找位置：根据值和父节点的值大小，判断是左子还是右子。
