...
函数表使用了一个数组来存储类声明的每一个函数的指针，大部分语言把这个称为 “virtual table”(虚函数表)，Swift 里称为 “witness table”，每一个类都会维护一个函数表，里面记录着类所有的函数。

如果父类函数被`override`的话，表里面只会保存被`override`之后的函数。

一个子类新添加的函数都会被插入到这个数组的最后，运行时会根据这一个表去决定实际要被调用的函数.
