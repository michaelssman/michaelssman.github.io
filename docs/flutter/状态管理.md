...
一般整个页面是`StatelessWidget`，里面哪个变的是`StatefulWidget`。不需要`setState(() {}`的就用`StatelessWidget`，需要的就用`StatefulWidget`。
...
`StatefulBuilder` 是 Flutter 中的一个工具，它用于创建一个局部的可变状态，不需要为了仅仅更新一小部分 UI 而创建一个完整的 `StatefulWidget`。
...
在这个例子中，我们在 `MyHomePage` 类中创建了一个按钮，当按下时会显示一个对话框。对话框使用 `StatefulBuilder` 来创建一个可变的计数器。当用户点击 "Increment" 按钮时，计数器的值会增加，并且通过调用 `setState` 函数，告诉 `StatefulBuilder` 需要重建，以便更新显示的计数器值。

注意：上述代码中的 `(context as Element).markNeedsBuild();` 是一种强制更新 UI 的方法，但在实际的 Flutter 开发中，我们通常不推荐这样做，因为它不符合 Flutter 的最佳实践。正确的做法应该是在 `StatefulBuilder` 的 `builder` 方法中调用 `setState` 函数来更新状态，告诉 `StatefulBuilder` 需要重建其子树，从而更新计数器的显示值。

## 访问和修改外部状态

状态提升，把自己写的组件里面的属性提升到父组件，父组件传进来。
