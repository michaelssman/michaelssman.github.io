...
不管是 JPEG 还是 PNG 图片，都是一种压缩的位图图形格式。PNG图片是无损压缩，并且支持 alpha 通道，JPEG 图片是有损压缩，可以指定 0-100% 的压缩比。在苹果的 SDK 中专门提供了两个函数用来生成 PNG 和 JPEG 图片：
...
- 获取图片二进制数据
- 创建一个`CGImageRef`对象
- 使用`CGBitmapContextCreate()`方法创建一个上下文对象context
- 使用`CGContextDrawImage()`方法绘制到上下文
- 使用`CGBitmapContextCreateImage()`生成`CGImageRef`对象。
- 最后使用`imageWithCGImage()`方法将`CGImage`转化为`UIImage`。
...
加载之前时是`22M`，加载之后是 `17M`，节省了大约 `5M`的内存空间。
...
在`SDWebImage`中，解码的队列 `_coderQueue.maxConcurrentOperationCount = 1`就是一个**串行队列**。这样就解决了`多图片异步解码`时，`线程爆炸`问题。
...
着色器渲染过程：在渲染过程中，必须存储2种着色器：顶点着色器、片元着色器。顶点着色器是第一个着色器、片元着色器是最后一个。顶点着色器处理顶点、片元着色器处理像素点颜色。
...
1. 先确定顶点信息 交给顶点着色器
   ...
   GPU获取图片的坐标（Frame），将坐标交给顶点着色器`VertexShader`(顶点变换计算)
...
4. 执行片元着色器（高度并发GPU）
   ...
   片元着色器`FragmentShader`计算每个像素点的最终显示的颜色值，根据纹理坐标获取每个像素点的颜色值(如果出现透明值需要将每个像素点的颜色*透明度值)
