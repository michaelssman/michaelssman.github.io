...
struct __main1_block_impl_0 {
  ...
  __main1_block_impl_0(void *fp, struct __main1_block_desc_0 *desc, int _a, int flags=0) : a(_a) {
    ...
    impl.Flags = flags;
    impl.FuncPtr = fp; //编程思想：函数式。先保存block函数实现，在block调用的时候执行。
    Desc = desc;
  }
};
...
在 Block 的底层实现中，`__Block_byref_xxx` 是一个自动生成的结构体名称，`byref` 是 **by reference** 的缩写，表示这个结构体**通过引用去捕获变量**。
...
在 Block 被复制到堆时调用，用于处理 Block 捕获的各种类型变量的内存管理。例 是否有加__block
...
static struct Block_byref *_Block_byref_copy(const void *arg) {
    ...
    if ((src->forwarding->flags & BLOCK_REFCOUNT_MASK) == 0) {
        ...
        copy->flags = src->flags | BLOCK_BYREF_NEEDS_FREE | 4;
      	...
      	//更新 forwarding 指针：
        copy->forwarding = copy; // 堆上 byref 的 forwarding 指向自己
        src->forwarding = copy;  // 原来栈上的 byref 的 forwarding 现在也指向堆上的 byref
        // 拷贝 size
        copy->size = src->size;
