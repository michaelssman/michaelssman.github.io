...
Swift 中创建类和结构体的实例时必须为所有的存储属性设置一个合适的初始值。所以类 LGPerson 必须要提供对应的指定初始化器，同时我们也可以为当前的类提供便捷初始化器。
...
iOS开发的语言不管是OC还是Swift，**后端**都是通过LLVM进行编译的，如下图所示：

![image-20220127135838384](类与结构体.assets/image-20220127135838384.png)
...
![image-20220127140601763](类与结构体.assets/image-20220127140601763.png)
