...
**无法使用 break 或 continue 跳出或者跳过循环**，使用 return 只能退出当前一次循环的执行体。
...
`partition(by belongsInSecondPartition: (Element) throws -> Bool)`将数组以某个条件分组。

数组前半部分都是不符合条件的元素，数组后半部分都是符合条件的元素。
...
- 互斥性 ： 集合中任何两个元素都认为是不相同的，即每个元素只能出现一次。
- 无序性 ： 集合中每个元素的地位都是相同的，元素之间是无序的。

### Swift 里面的集合

集合类型写做`Set<Element>`，Element是Set要储存的类型。
...
- 字典储存无序的、互相关联的**同一类型的键**和**同一类型的值**的集合
