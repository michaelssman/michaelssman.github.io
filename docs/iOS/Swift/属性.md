...
存储属性是一个作为特定类和结构体实例一部分的**常量或变量**。存储属性要么是变量存储属性（var 关键字引入）要么是常量存储属性（let 关键字引入）。
...
计算属性不存储值，提供 getter 和 setter 来修改和获取值。计算属性本质是get和set。计算属性不占用内存空间。
...
需要注意的一点是在**初始化期间设置属性时不会调用 willSet 和 didSet 观察者**；只有在为完全初始化的实例分配新值时才会调用它们。运行下面这段代码，你会发现当前并不会有任何的输出。 
