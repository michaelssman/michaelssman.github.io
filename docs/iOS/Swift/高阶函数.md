...
假设我们有一个`String`类型的`Array`，现在我们对集合当中的每一个元素都是小写的。
...
let result5 = numbers2.reduce(10, +)
...
```swift
let result = [1, 2, 3, 4, 5].reduce([Int]()){//初始值是空数组[Int]()
    return [$1] + $0//$1数组中的元素 $0第一个参数
}
