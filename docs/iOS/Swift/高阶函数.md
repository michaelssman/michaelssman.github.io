...
这些函数常常用来作用于`Array`、`Set`、`Dictionary`中的每一个元素
...
1. **对每个元素进行转换**（类似 `map`）。
2. **自动解包可选值（Optional）**，并过滤 `nil`。
3. **扁平化嵌套数组**（将 `[[T]]` 变成 `[T]`）。

`flatMap` 中的闭包的参数同样是 `Sequence` 中的元素类型，但其返回类型为`SegmentOfResult.Element` 的数组。

`SegmentOfResult` 的类型其实就是 `Sequence`。
