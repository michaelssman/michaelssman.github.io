...
**泛型在类上，表明整个类内部都可以使用。泛型在方法上，表明在方法内使用。**
...
一旦指定了一个类型形式参数，你就可以用它定义一个函数形式参数的类型，或者用它做函数返回值类型，或者做函数体中类型标注。在不同情况下，用调用函数时的实际类型来替换类型形式参数。
...
类型形式参数永远用大写开头的驼峰命名法，以指明它们是一个类型的占位符，不是一个值。
...
例如，Swift的 Dictionary 类型在可以用于字典中键的类型上设置了一个限制。Dictionary键的类型必须是是可哈希的，也就是说，它必须提供一种使其可以唯一表示的方法，以便它可以检查字典中是否包含一个特定键的值。没有了这个要求， Dictionary 不能区分该插入还是替换一个指定键的值，也不能在字典中查找已经给定的键的值。
...
找数组中某一个素的下标，元素T可以是string，int等等。
