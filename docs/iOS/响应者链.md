...
一个APP在前台静止着，此时，用户手指点击了一下APP界面，那么过程就是下面这样的：

1. 触摸到硬件(屏幕)，发生触摸事件后，系统捕捉此事件，此事件会被`IOKit`包装成UIEvent，通过`mach_Port`传给正在活跃的APP，`mach_port`和`source1`是一一对应的（key和value），`source1`唤醒`RunLoop`，然后将事件Event分发给`source0`，然后由`source0`来处理。
2. 将此UIEvent对象加入当前应用程序的事件队列中。
3. **UIApplication**对象从队列中一个一个取出来进行分发
4. 首先分发给UIWindow对象。
5. UIWindow再将事件逐步往下层找，即UIView。
6. UIView首先看触摸点是否在自己身上，自己能否处理事件。如果能，那么继续寻找子视图。
7. 遍历子控件，重复以上两步。直到找到最深层的子视图。
8. 如果没有找到，那么自己就是事件处理者。也就是第一响应者对象`first responder`。
9. 如果自己不能处理，那么不做任何处理。
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *view = [super hitTest:point withEvent:event];
  	// 获取屏幕上点击的某一个view，里面使用superView
    NSLog(@"view11111 = %@", view);
    if (![view isKindOfClass:NSClassFromString(@"UITableViewCellContentView")] && [view.superview isKindOfClass:[CCChatBaseCell class]]) { // 寻找到最合适的view
      return view;
    }
...
事件响应链机制主要就是查找事件源（调用hitTest...查找）和反馈给系统处理（调用.nextResponse)。

只需在点击tableview的事件反馈给appdelegate处理之前关闭键盘就可以了，这样既关闭了键盘，又不会由于覆盖tableview封装的手势而导致cell的手势操作异常问题。
...
在含有轮播图和tableView的整个的父view上添加方法
