...
首先触摸到硬件(屏幕)，屏幕表面的事件会被`IOKit`包装成UIEvent，通过`mach_Port`传给正在活跃的APP，`mach_port`和`source1`是一一对应的（key和value），`source1`唤醒`RunLoop`，然后将事件Event分发给`source0`，然后由`source0`来处理。

1. 发生触摸事件后，系统捕捉此事件，并为此事件创建一个UIEvent对象。
