...
1. 发生触摸事件后，首先系统会捕捉此事件，并为此事件创建一个UIEvent对象。
...
利用响应链机制来解决，事件响应链机制主要就是查找事件源（调用hitTest...查找）和反馈给系统处理（调用.nextResponse)，所以现在只需在点击tableview的事件反馈给appdelegate处理之前关闭键盘就可以了，这样既关闭了键盘，又不会由于覆盖tableview封装的手势而导致cell的手势操作异常问题。
