...
1. 读取文件
   ...
   使用 +imageWithContentsOfFile:(使用Image I/O创建CGImageRef内存映射数据)方法从磁盘中加载一张图片，此时，图像尚未解码。在这个过程中先从磁盘拷贝数据到内核缓冲区，再从内核缓冲区复制数据到用户空间
...
>
> ![CPU和GPU协作](图片加载解码渲染.assets/16cfbeb485755223~tplv-t2oaga2asx-zoom-in-crop-mark:1304:0:0:0.awebp)
