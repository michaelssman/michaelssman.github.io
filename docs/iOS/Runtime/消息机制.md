...
调用方法 = 消息发送：`objc_msgSend(消息接收者, 消息的主体(方法名sel + 方法参数));`
...
1. cmp 比较指令，判断**消息接收者**receiver是否存在
2. 通过**消息接收者**receiver的isa找到对应的class
...
类也是对象，objc_class继承objc_object。

方法有方法缓存，方法缓存存在cache_t中，cache_t存在类对象objc_class结构体中。
...
缓存未命中就去找methodList，从类的data_bits中的data数据中找（data数据中有 属性列表 成员变量 方法列表 协议列表）。
...
方法列表是一个哈希表，里面的每一个index是经过哈希函数计算得到。index对应一个链表。
...
类的Cache查找和Method list查找都没有找到，则进行动态方法解析

## 消息转发机制 动态特性

如果没有查到方法的 IMP 指针，则进行消息转发机制
...
`resolveMethod_locked(id inst, SEL sel, Class cls, int behavior)`拦截处理。
...
调用 `resolveInstanceMethod:`或`resolveClassMethod: `，为该Class动态添加方法实现。如果实现了，则调用并返回YES，那么重新开始objc_msgSend流程。这一次对象会响应这个SEL选择器，因为它已经调用过`class_addMethod`。

class_addMethod本质上是在方法列表中建立SEL和新的IMP关系，新的IMP是自己做的。动态解析的方法，同样会缓存在cache_t。
...
重定向 forward（obj_msgForward 方法的转发）找一个**备用的接收者**来处理消息。

如果第一层转发返回 NO ，则会进行第二层转发，调用` forwardingTargetForSelector: `，把调用转发到另一个对象，这是类级别的转发，调用另一个类的相同的方法。

**消息缓存在备用接收者里。**

实例方法：`- (id)forwardingTargetForSelector:(SEL)aSelector`

类方法：`+ (id)forwardingTargetForSelector:(SEL)aSelector`
...
1. `methodSignatureForSelector:`获得方法签名
   - 如果获取不到则直接调用`doesNotRecognizeSelector:`抛出异常。
   
   - 如果能获取到，则返回`NSMethodSignature`。
...
调用`_objc_msgForward`会跳过查找IMP的过程，直接触发消息转发。
