...
动态库在编译时并不会被拷⻉到⽬标程序中，**⽬标程序中只会存储指向动态库的引⽤**。程序运⾏时，动态库才会被真正加载进来。
...
动态库： 所有的代码和所有的符号都是放到一起，编译链接的最终产物，不能再链接修改，不能再合并。
...
系统的UIKit.Framework不需要拷⻉到⽬标程序中

⾃⼰做的 Framework 哪怕是动态的，最后也要拷⻉到 App 中（App 和 Extension 的 Bundle 是共的），因此苹果⼜把这种 Framework 称为 Embedded Framework。
...
build_xcframework() {
    ...
    # Create XCFramework (包含 dSYM)
    ARCHIVES="/Users/michael/Desktop/HHUtils"
    # 用 -debug-symbols 参数把 dSYM 一起打包进 .xcframework。
    # -debug-symbols：必须使用绝对路径。否则会出现error: the path does not point to a valid debug symbols file。
    # -debug-symbols 参数要求传入的是 .dSYM。
    xcodebuild -create-xcframework \
      -framework "$IOS_ARCHIVE_PATH/Products/Library/Frameworks/$scheme_name.framework" \
      -debug-symbols "$ARCHIVES/build/ios_devices.xcarchive/dSYMs/$1.framework.dSYM" \
      -framework "$IOS_SIMULATOR_ARCHIVE_PATH/Products/Library/Frameworks/$scheme_name.framework" \
      -debug-symbols "$ARCHIVES/build/ios_simulator.xcarchive/dSYMs/$1.framework.dSYM" \
