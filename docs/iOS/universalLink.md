...
`targets -> Signing&capabilities -> Associated Domains`，在其中的`Domains`中填入你想支持的域名。

如：配的universalLink是https://m.nmkjxy.com/app/，则`Associated Domains`设置`applinks:m.nmkjxy.com`。
