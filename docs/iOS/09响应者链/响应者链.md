...
在含有轮播图和tgableView的整个的view上添加方法
...
响应链是从最合适的view开始传递，处理事件传递给下一个响应者，响应者链的传递方法是事件传递的反方法，顺序倒着来，如果所有的响应者都不处理事件，则事件被丢弃。我们通常用响应者链来获取上几级响应者，方法是`UIResponder`的`nextResponder`方法。
