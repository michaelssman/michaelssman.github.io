...
URI 另一个更常用的表现形式是 URL`统一资源定位符（Uniform Resource Locator）`。因为它经常出现在浏览器的地址栏里，所以俗称为“网络地址”，简称“网址”。URL是 URI 的一个子集，通常不会做严格的区分。
...
URI 本质上是一个字符串，作用是唯一地标记资源的位置或者名字。

![图片](assets/640-20220902184224676.jpeg)

上面这个图片就是一个完整的URI，下面详细拆解一下它的结构。

- **scheme** 协议名，表示资源应该使用哪种协议来访问。`http`表示使用 HTTP 协议。另外还有`https`表示使用经过加密、安全的 HTTPS 协议。此外还有其他的 scheme，例如 ftp、ldap、file、news 等。

- **://** 分隔符，三个特定的字符`://`，它把 scheme 和后面的部分分离开。没有特定的意义。

- **user:passwd@** 身份信息，表示登录主机时的用户名和密码，现在已经不推荐使用这种形式了，因为它把敏感信息以明文形式暴露出来，存在严重的安全隐患。

- **host:port** ：表示资源所在的主机名，通常的形式是`host:port`，即主机名加端口号。
...
IP地址用来标志一台计算机，但是一台计算机上可能提供多种应用程序，使用端口来区分这些**应用程序**。
