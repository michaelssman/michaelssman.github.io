...
SOCKET翻译过来叫套接字，是网络编程最基础的接口。

SOCKET被设计成文件描述符。SOCKET的的核心理念是把复杂的网络通信抽象成简单的打开读写关闭等，类似于操作文件的接口，让程序员不用关心底层网络协议的复杂细节，只需要调用几个函数，就能实现网络通信。极大降低了网络编程的门槛，没有SOCKET就没有今天的互联网。

SOCKET本质上是一个通信端点，它由三个参数组成：

IP地址：IP地址告诉你数据要发到哪台电脑。
端口号：一台电脑上可能同时运行几十个程序，端口号用来标识电脑上的某个具体程序。
传输协议类型：来确定使用UDP传输还是TCP传输

SOCKET等于IP地址加端口号加协议类型，这三个信息组合起来，就能精确定位网络上的某个程序
你的程序想要通过网络通信，创建一个SOCKET，连接到对方的SOCKET，数据就能传过去了，你不需要知道
数据包是怎么在互联网上路由的，不需要理解什么三次握手，滑动窗口拥塞控制这些复杂的事儿，SOCKET帮你搞定了。从技术角度看，SOCKET是操作系统内核实现的通信机制，它构建在传输层协议之上，既支持面向连接的TCP方式，也支持无连接的UDP方式。

创建SOCKET时，系统会分配一个文件描述符，使得网络io可以像文件读写那样操作，这种设计抽象了网络硬件的复杂性，为应用程序提供了统一的网络访问接口。

在实际开发中socket编程遵循固定的模式：

- 服务器端依次调用SOCKET bind listen accept
- 客户端调用SOCKET connect
- 数据收发使用read write或send receive函数
...
**注意**：WebSocket 是状态化协议，服务器需管理大量持久连接，对连接管理能力要求较高；
