...
攻击一个网站，在登录页面中，账号我们可以这样写：`abcdefg'or'1'='1`密码也是一样的这么写，这样就是我们万能密码的形式。我们在不知道账号密码的情况下使用万能密码就成功登录了。

那它的原理到底是什么？

首先，我们需要明白登录执行过程中在数据库里面长什么样子，那这段代码就是数据库的查询语句（select * from 表名 where 用户名 = 账号 and 密码 = 密码）。这个查询语句中，只要说在这里面它满足这两个条件为真（用户名 = `真 `and 密码 = `真`），那么就代表整个语句是真，那么数据库就会返回登录成功。所以我之前写的这个万能密码表达的就是真的意思（where 用户名 = `abcdefg'or'1'='1` and 密码 = `abcdefg'or'1'='1`），这个abcdefg可以不写，那万能密码就长这个样子，因为1是恒等于1的，它永远为真。那么就代表登录成功了。

刚刚我们在网站输入账号密码环节中，我们输入框里面输入的东西就不是正常内容，而输入的是SQL语句。这个SQL语句可以骗过数据库，达到让数据库去执行增删改查的效果。
