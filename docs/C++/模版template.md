...
**做模版类的时候，代码都得放到头文件中，不能在源文件。**C++编译的时候会生成类，所以必须是可见的。
...
这样一个函数就要针对每一种类型都要写一个同名的函数重载，工作都是重复的。（重复是万恶之源）。

可以用C++的模版，做一个函数模版。C++在**编译的时候**根据模版动态的生成需要的函数。
...
```c++
template<class T, int size = 10>//模版常量size
class Array
{
    //enum {size = 10};//枚举常量
    T array[size];
