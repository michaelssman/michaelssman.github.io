...
优点：在预处理阶段进行了文本替换，避免函数调用的开销，所以就快。

缺点：因为是普通文本替换，容易出错。

所以C++提供更好的方法：内联函数，在函数前面加`inline `。
...
C++`inline`函数取代C语言的宏函数，inline更安全。
...
内联函数不需要源文件，只写在头文件里。不能写成像普通函数那样 一个声明 一个定义。
...
比较小的函数写成内联函数，例如简单的构造函数。比较大的写了也没有用，函数里面代码很多的时候，C++编译器也不做内联。
...
Stash和Stack小的成员函数做成内联函数，都在.h文件中，不需要源文件。运行速度会更快。
