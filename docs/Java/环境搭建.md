...
1. **`docker ps -q`**
   - `-q` 参数表示只输出容器的 **ID**（安静模式）
   - 该命令会列出所有正在运行容器的 ID（例如：`ded8087b0451 eb44f9063118 e762c6af063d`）
2. **`docker restart`**
   - 将上一步获取的容器 ID 作为参数传递，批量重启所有容器
...
- `docker run` 启动容器
- `--name`：为容器命名，容器名字就叫mysql-container。
- `-d`：创建一个守护式容器**在后台运行**，后台运行容器（detached模式）。
- `-p`：**映射端口。将容器的3306端口映射到宿主机的3306端口，可以通过服务器的 IP 地址和端口 3306 访问 MySQL。**
- `-v /my/own/datadir:/var/lib/mysql`：数据持久化，宿主机目录挂载到容器内MySQL数据目录
