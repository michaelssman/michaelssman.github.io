...
可以通过以下命令连接到 MySQL 容器：

```bash
docker exec -it mysql-container mysql -u root -p
```

输入你设置的密码 `my-secret-pw`。
...
- `/my/own/datadir`：替换为你希望用于存储 MySQL 数据的本地目录。

### 5. 端口映射

如果你在启动容器时没有进行端口映射，那么 MySQL 只在容器内部的默认端口 3306 上运行。

如果需要从主机访问 MySQL，可以映射端口：
...
**2003 - Can't connect to MySQL server on '47.120.67.123' (10061 "Unknown error")**
