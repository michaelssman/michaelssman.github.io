...
添加到`heima-leadnews\heima-leadnews\heima-leadnews-common\src\main\resources\META-INF\spring.factories`自动配置中
...
存储：一次性的把所有的敏感词存储到了多个map中，每一个字都是一个key，就是下图表示这种结构
