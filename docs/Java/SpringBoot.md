...
Spring Boot的启动类的作用是启动Spring Boot项目。通过启动类的main方法开始运行，通过SpringApplication.run方法启动Spring Boot，在它的底层：

1. 首先加载Spring Boot的一些基本的初始信息
2. 读取我们的配置文件的信息
3. **创建Spring容器**。因为Spring Boot包含了Spring，所以还会创建Spring容器，从而创建一个一个的bean对象
4. **加载自动配置类**。在程序中我们通常会配置很多的starter，即自动配置了很多默认的配置信息。所以可以通过添加starter的依赖开箱即用。还会通过@ConditionalOnXXX的注解来决定自动配置类是否生效。
5. **启动内置的Tomcat**。Spring Boot是一个jar包，但是可以通过Web的方式启动应用，就是因为内置了Web服务器Tomcat。
6. 发布容器启动完成的事件。
