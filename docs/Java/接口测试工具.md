...
Reqable 可以创建普通 HTTP 请求、管理环境变量、设置 Basic/Bearer 认证，并查看请求与响应的 Header、Body、耗时和状态码。调试认证接口时建议按以下方式组织。

### 1、环境变量

不要把密码、client secret 或 Token 直接写进共享请求。可以定义：

```text
base_url=http://127.0.0.1:8080
username=<测试用户名>
password=<测试密码>
access_token=<登录后临时保存>
oauth_client_id=<部署环境提供>
oauth_client_secret=<部署环境提供>
```

秘密变量只保存在 Reqable 私有环境，不导出到 Git 或公共文档。

### 2、选择正确的 Body 类型

| 接口类型 | Reqable Body |
| --- | --- |
| REST JSON | JSON，并设置 `Content-Type: application/json` |
| OAuth Token | Form URL Encoded，并设置 `application/x-www-form-urlencoded` |
| 文件上传 | Multipart Form，文件字段名必须与 Controller 一致 |
...
- Bearer Token：在 Auth 中选择 Bearer，值填 `{{access_token}}`。
- OAuth client credentials：选择 Basic Auth，分别填写 client id 和 client secret，让工具生成 Header。
- 不要同时手工添加第二个 `Authorization` Header。
- 调用公开登录接口时不要携带已过期的 Bearer Token，否则资源服务器仍可能先拒绝该无效 Token。

### 4、根据状态码定位

| 状态 | 含义与排查方向 |
| --- | --- |
| 400 | JSON/Form 类型错误、参数校验失败、grant_type 不支持 |
| 401 | 缺少、过期或篡改 Token，issuer/audience/JWKS 不一致，或 OAuth 客户端认证失败 |
| 403 | 身份已建立但不满足授权规则，或安全链/CSRF 配置不正确 |
| 404 | base URL、context path、Gateway 路由或 StripPrefix 配置错误 |
| 500 | 请求可能已经通过认证，应继续查看后端业务异常，不能直接归因于 Token |

### 5、保留可复现证据

问题记录中可以保留请求方法、URL 模板、Content-Type、非敏感参数名、HTTP 状态和响应结构。必须删除真实密码、Authorization Header、Cookie、client secret、JWT 和 RSA 密钥。

`hhjava` 注册、登录、Bearer、client credentials 和 JWKS 的完整请求示例见 [hhjava 认证授权与配置安全整改实战](./认证授权和网关/hhjava认证授权与配置安全整改实战.md)。
