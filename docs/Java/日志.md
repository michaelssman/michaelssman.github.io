...
| Logback、Log4j2 | 日志的具体实现，负责级别过滤、格式化、控制台和文件输出等；一个应用通常只选其中一种 |
...
Spring Boot 的常用 Starter 默认引入 Logback，未排除默认日志依赖时可直接使用，不用再单独添加日志库。
...
对于分布式系统，可以使用集中日志收集系统，例如 ELK：Elasticsearch 负责存储与检索，Logstash 负责采集处理，Kibana 负责查询和展示。它解决跨服务查日志的问题，不替代应用内的 SLF4J、Logback 或 Log4j2；小型本地实验不必先部署整套收集系统。

## Spring Boot 集成 Log4j2：从依赖到文件输出

### 实现效果与前置准备

本节在本机已有、能够正常启动的 Maven + Spring Boot 3.x、Java 17 学习应用中操作，使用 IDEA 启动，不需要另装日志服务器。依赖版本由项目的 Spring Boot 父 POM 或 BOM（Bill of Materials，统一管理依赖版本的清单）管理，并随项目维护安全更新。

目标是：控制台和文件同时输出、默认记录 `INFO` 及以上、按天或约 10 MB 归档，并在滚动时清理达到 14 天的归档文件。这些数值是教学示例，不是生产环境的统一标准。

这是通用的可选配置方案，不表示 hhjava 已经添加这些配置或启用了异步日志；具体项目是否生效，还要核对实际依赖、资源文件、启动参数和外部配置。

### 第一步：引入实现，并排除默认日志

在需要启动的服务 `pom.xml` 的 `<dependencies>` 中配置下面两项；原有业务依赖不变。如果父 POM 或当前 POM 已经声明了相同依赖，就在原声明处合并，不要重复添加。
