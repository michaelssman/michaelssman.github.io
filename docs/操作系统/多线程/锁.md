...
很多锁之间其实并不是并列的关系，而是一种锁下的不同实现。

### 1、自旋锁（spin lock）

它的特点是在线程等待时会一直轮询，处于**忙等**状态。
...
其它锁出现优先级反转后，高优先级的任务不会忙等，**没有占用时间片**，所以低优先级任务能够进行下去，从而释放掉锁。
...
同一个线程可以重复获取递归锁，不会死锁（例：synchronized）。
...
1. `[condition lock];`//一般用于多线程同时访问、修改同一个数据源，保证在同一时间内数据源只被访问、修改一次，其他线程的命令需要在**lock** 外等待，只到 **unlock** ，才可访问
...
条件变量，类似信号量，提供线程阻塞与信号机制，可以用来阻塞某个线程，等待某个数据就绪后唤醒线程。

一个条件变量总是和一个互斥量搭配使用。

而 NSCondition 其实就是封装了一个**互斥锁**和**条件变量**，互斥锁的 lock/unlock 方法和后者的 wait/signal 统一封装在 NSCondition 对象中，暴露给使用者。
...
- `unlockWithCondition:`先解锁，再修改 condition 参数的值。
- `lockWhenCondition:`符合 condition 的值再上锁。
...
NSRecursiveLock 和 @synchronized 一样，是一个递归锁。

**NSRecursiveLock无法进行多线程的可递归，而synchronized可以多线程可递归。**
...
synchronized解决了NSRecursiveLock的不可多线程问题。
